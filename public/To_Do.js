...
var ref = firebase.database().ref("To_DO List");
...
function todoValue() {
  ...
  // Firebase Database || Set Data To Firebase
  ref.child(key).set(toDos);
  inputVal.value = " ";
}

// Getting Data From Firebase 
  // And Setting It On HTML
...
delBtn.setAttribute("id", data.val().liKey);
...
editBtn.setAttribute("id", data.val().liKey);
...
function delli(lis) {
  // console.log(lis.id)
  ref.child(lis.id).remove(); // Remove Li From Firebase
...
function edit(edt) {
  ...
  ref.child(edt.id).set(liEdit); // Edit Li In Firebase
